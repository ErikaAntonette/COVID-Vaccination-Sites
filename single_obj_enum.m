...
for L = 2:4
tic
eps = 5000; %distance in meters for coverage obj fcn
...
for j = 1:size(combs,1)
    if L == 1
     Cost1(j) = sum((w1+w2)'.*(Distance(combs(j,:)',:)));
    else
        Cost1(j) = sum((w1+w2)'.*min(Distance(combs(j,:)',:)));
    end
end
...
% end
% 
% [X2,Y2]=mink(Cost2,1); %% change 1 to get top N sites
% bestcombs2 = combs(Y2,:);
